...
## 2026-02-19: Card Grid 详情 Banner 占比可调（可向下占据更多空间）

### 需求

为 `card-grid` 的详情面板增加 banner 占比控制，使 banner 在详情卡中可按比例占据更多垂直空间（向下扩展）。

### 实现

1. 在 `card-grid` 详情渲染时接入新的卡片级字段：
   - `features.cards[i].detailBannerRatio`
   - 默认值 `60`，并在渲染层统一做 `20~100` 的安全约束（沿用已有 shrink 比例约束逻辑）

2. 在模板层打通到通用详情渲染：
   - `js/templates.js` / `docs/js/templates.js`
   - `cardGridTemplate()` 调用 `renderDetailContent()` 时新增：
     - `bannerShrinkRatio: detailBannerRatio`
     - `detailShrinkPath: features.cards.${i}.detailBannerRatio`
   - 同时显式传入 `bannerUseNaturalSize: false`，保证 card-grid 使用“高度占比”模式，而不是 TIL 的自然尺寸图片模式。

3. 复用已有编辑器滑杆：
   - `section-renderer.js` 已有 `initializeDetailShrinkControls()`，会自动识别 `data-detail-shrink-path`
   - 因为 card-grid 现已输出该路径，编辑模式下会自动出现 `Banner height` 滑杆并保存到 material。

4. 新建卡片默认值补齐：
   - `js/editor.js` / `docs/js/editor.js` 的 `createDefaultCollectionItem('card-grid')` 增加 `detailBannerRatio: 60`
   - 确保新增卡片开箱可调。

### 结果

- Card Grid 每张卡片详情的 banner 支持独立调节占比，可明显“向下占据更多空间”。
- 支持离线/在线一致保存（走现有 ModeManager 路径）。
- 对旧数据兼容：旧卡片无该字段时仍按默认值渲染，不会报错。

### 补充修复（2026-02-19）：前端编辑界面可视化滑杆入口

用户反馈编辑界面没有看到可视化调节入口。排查后将 banner 占比滑杆直接并入详情图片的 `img-position-controls` 控制条（与 X/Y/Zoom 同一区域），避免原先控件在详情布局中不易被看到。

实现点：
- `js/editor.js` / `docs/js/editor.js`
  - 在 `enableDetailBannerToggles()` 中新增 `detail-banner-ratio-wrap`：
    - `B`（Banner）滑杆：`20~100`
    - 百分比实时显示
    - `Reset` 按钮回到 `60`
  - 拖动时实时更新 `--detail-banner-shrink-ratio`（所见即所得）
  - 释放后保存到 `index.features.cards[i].detailBannerRatio`（含在线 patch）
- `css/styles.css` / `docs/css/styles.css`
  - 增加 `detail-banner-ratio-wrap` 与数值样式，确保在控制条中可读可用

结果：编辑模式下，打开 card-grid 详情后可直接在图片控制条中看到并拖动 banner 占比滑杆。

### 补充修复（2026-02-19）：滑杆拖动无视觉变化

问题根因：`.detail-banner-media` 原样式固定 `height: 220px`，而滑杆只在改 `max-height`，导致多数场景看起来“滑了但没变化”。

修复：
- `css/styles.css` / `docs/css/styles.css`
  - 将 `.detail-content-wrap--banner-shrunk .detail-banner-media` 的高度改为：
    - `height: clamp(120px, calc(var(--detail-banner-shrink-ratio, 60) * 1vh), 60vh)`
  - 保留上下限，并恢复 `background-size: cover` 以匹配 card-grid banner 视觉

结果：拖动 Banner 占比滑杆会实时改变 banner 高度，视觉变化明显。

### 回归修复（2026-02-19）：TIL / RIL Learn More 展示异常

现象：调整 card-grid banner 占比后，`text-image-left/right`（TIL/RIL）进入 Learn More 时出现图片与文字布局异常（图片显示区域被错误高度约束）。

根因：
- 高度规则作用范围过宽，命中了自然尺寸 banner（`.detail-banner-media--natural`）。
- 同时 `styles.css` 重新编译后，相关 custom 样式在源文件 `css/input.css` 中并不完整，导致运行时样式不稳定。

修复：
1. 在 `css/input.css` 里补齐并固化相关样式，再重新编译输出：
   - `card-grid` 专用高度规则改为：
     - `.detail-content-wrap--banner-shrunk .detail-banner-media:not(.detail-banner-media--natural)`
   - `TIL/RIL` 自然尺寸规则恢复：
     - `.detail-banner-media--natural`
     - `.detail-banner-img-natural`
   - 同步补齐：
     - `.img-position-controls .detail-banner-ratio-wrap`（可视化滑杆样式）
     - `.detail-shrink-controls`（旧控件样式）
2. 重新构建并同步：
   - `npx tailwindcss -i ./css/input.css -o ./css/styles.css`
   - `cp ./css/styles.css ./docs/css/styles.css`

结果：
- card-grid 仍可调更高 banner 占比（上限 85vh）。
- TIL/RIL 的 Learn More 恢复正常：图片自然尺寸显示与文字布局不再被误伤。

### 回归修复（2026-02-19）：TIL/RIL Learn More 不按历史位置与比例回放

现象：`text-image-left/right` 在 Learn More 详情态中，图片位置与缩放不再按历史 `positionDetail/positionCollapsed` 设置值显示。

根因：`til-detail-natural-img` 关键样式缺失，导致 JS 写入的 `transform/objectPosition` 无法按预期生效。

修复：
- 在 `css/input.css` 追加并固化 TIL/RIL 专用样式：
  - `.til-image-wrapper { position: relative; overflow: hidden; }`
  - `.til-detail-natural-img { position:absolute; left/top:50%; transform:translate(-50%,-50%) scale(...); object-fit:none; ... }`
- 重新构建并同步：
  - `npx tailwindcss -i ./css/input.css -o ./css/styles.css`
  - `cp ./css/styles.css ./docs/css/styles.css`

结果：TIL/RIL 详情态图片恢复按保存的 `positionDetail/positionCollapsed` 回放，缩放与位置与历史设置一致。

### 回归修复（2026-02-19）：TIL/RIL details 图片应为裁切填充 + details 态切换丢字

用户反馈两点：
1. TIL/RIL 进入 details 后图片被按原始比例显示（不符合期望，应为裁切填充）  
2. 在 details 状态切换 section 仍会出现文字内容丢失

修复内容（`js/section-renderer.js` 与 `docs/js/section-renderer.js`）：

1. **details 图片渲染改回 crop/fill**
   - 移除 details 态下强制创建 `.til-detail-natural-img` 的逻辑
   - 改为在 details 态同样使用 `background-image`，并按 `positionDetail` 回放：
     - `backgroundSize = \`${scale}%\``
     - `backgroundPosition = \`${x}% ${y}%\``
   - 进入 details 时若存在旧的自然图节点会清理，避免叠层干扰

2. **details 模式切换 section 前强制提交编辑**
   - 在 `switchToIndex()` 开头增加：
     - 若当前焦点在该 section 的 `contenteditable` 元素上，先 `blur()`
   - 确保编辑中的 detail 文本先触发保存，再执行切换动画与数据切换

3. **details 位置保存兜底**
   - `saveCurrentTILData()` 在 `detailState === expanded` 时：
     - 优先读取自然图旧字段（兼容历史）
     - 若无自然图，则从 `backgroundPosition/backgroundSize` 读取并写入 `images.positionDetail`

结果：
- TIL/RIL details 图片恢复“裁切填充”显示，并按已有 `positionDetail` 参数定位与缩放。
- details 态切换 section 时，文字编辑不再轻易丢失。

### 回归修复（2026-02-19）：card-grid banner 高度/缩放刷新后回退

现象：在 card-grid 详情中调整 banner 高度（H）与缩放后，刷新后出现部分回退。

根因与修复：

1. **H（detailBannerRatio）只在 `change` 保存，拖动中可能未落库**
   - 文件：`js/editor.js` / `docs/js/editor.js`
   - 在 `enableDetailBannerToggles()` 的 ratio 滑杆新增 `cacheRatio()`：
     - `input` 事件实时写入 `index.<ratioPath>` 并 `updateMaterialInMemory`
     - `change` 事件仍保留快照与正式保存流程

2. **card-grid 图片位置历史路径不一致（`position` vs `imagePosition`）**
   - 文件：`js/section-renderer.js` / `docs/js/section-renderer.js`
   - 在图片位置读取与保存链路中增加双路径兼容与镜像写入：
     - 读取：优先 `features.cards[i].position`，回退 `features.cards[i].imagePosition`
     - 保存：写入 `position` 的同时同步写入 `imagePosition`
     - 在线模式 patch 同步两条路径

结果：
- card-grid banner 高度和缩放在拖动后即进入内存 material，不依赖鼠标释放时机。
- 刷新后 `position/imagePosition` 任一路径都能正确回放，减少“保存了但丢了”的回退现象。

### 根本修复（2026-02-19）：Save All 后刷新数据丢失

现象：card-grid 的 banner 高度（B）、缩放（Zoom）及其他编辑数据在 Save All → 刷新后消失。

根因分析：
保存与加载路径存在根本性不一致：
1. **Save All 只保存到本地文件夹**（File System Access API），**不写入 IndexedDB**
2. **页面刷新后加载优先级**为：本地文件夹 > IndexedDB > material.json > 嵌入式
3. 当文件夹权限过期（浏览器重启、跨 session）时，文件夹加载失败
4. IndexedDB 中没有数据（因为 Save All 从未写入）
5. 最终回退到根目录 `material.json` — 该文件从不包含编辑器保存的数据

修复（3 处改动）：

1. **`js/editor.js` — `saveAll()` 增加 IndexedDB 同步写入**
   - 离线模式下，**先写 IndexedDB 再写文件夹**
   - IndexedDB 作为必备备份，文件夹保存为可选增强
   - 如果 File System Access API 不可用或用户取消文件夹选择，仍可保存到 IndexedDB
   - 成功/失败消息中明确告知两个存储通道的状态

2. **`js/mode-manager.js` — `loadMaterial()` 加载优先级调整**
   - 旧顺序：本地文件夹 > IndexedDB > material.json
   - 新顺序：**IndexedDB > 本地文件夹 > material.json**
   - 因为 IndexedDB 是每次保存操作（Ctrl+S、Save All）都一定会写入的存储，更可靠
   - 文件夹权限可能过期，不适合作为最高优先级

3. **同步 `docs/js/editor.js`**

结果：
- Save All 和 Ctrl+S 保存的数据均持久化到 IndexedDB
- 页面刷新优先从 IndexedDB 加载，不再依赖文件夹权限
- 文件夹保存仍然正常工作（用于导出独立查看器站点）
- Discard Changes 功能不受影响（清除 IndexedDB 后回退到 material.json）

### 加固修复（2026-02-19）：Save All 强制直接写 material.json

针对“保存提示成功但刷新后回退”的问题，新增硬落盘策略：
- 文件：`js/editor.js` / `docs/js/editor.js`
- 在 `saveAll()` 拿到已授权目录后，先执行直接写入：
  - `material.json`
  - `docs/material.json`（若存在则同步）
- 之后再执行原有 `saveCompleteOfflineSite()` 导出流程。
- 即使导出流程失败，只要 direct write 成功，也会给出“material.json 已直接写入”的提示，不再判定为完全失败。

目的：
- 让刷新读取的基础文件（`material.json`）一定包含最新编辑值，避免依赖浏览器草稿和权限状态。

### 修复（2026-02-19）：card-grid 封面与详情 banner 的 Zoom/位置分离

问题：
- `card-grid` 的封面图与详情 banner 使用同一条位置路径（`features.cards[i].position/imagePosition`），
  导致在详情里调 Zoom 会影响封面（或反向影响）。

改动：
1. `js/templates.js` / `docs/js/templates.js`
   - `renderDetailMedia()` 新增可选属性 `data-image-position-path`
   - `renderDetailContent()` 新增 `detailBannerPositionPath`
   - `card-grid` 详情 banner 改为独立路径：`features.cards[i].detailImagePosition`

2. `js/section-renderer.js` / `docs/js/section-renderer.js`
   - 图片位置控制优先读取元素上的 `data-image-position-path`
   - 对 `features.cards[i].detailImagePosition` 增加兼容回退：
     - 若新路径暂无值，首次可回退读取封面路径 `position/imagePosition`

结果：
- card-grid 外层封面与内层详情 banner 的 X/Y/Zoom 独立保存、独立回放。
- 旧数据首次使用时仍有平滑兼容，不会出现空值跳变。

...
## Bug修复 (2026-02-14) - Card Grid 详情文字无法编辑

### 问题现象
- 编辑模式下，Card Grid 卡片点击 "Learn more" 展开详情后，详情区域文字无法编辑（无 contenteditable、无法保存）。

### 根因分析
- `renderDetailContent()` 中当 `detailIsHtml === true`（Card Grid 使用）时，`detailMaterialAttr` 被设为空字符串，详情容器没有 `data-material`。
- 编辑器的 `enableInlineEditing()` 仅对 `[data-material]` 元素设置 contenteditable，因此该详情区域未被绑定为可编辑。

### 修复方案
- 在 `js/templates.js` 与 `docs/js/templates.js` 的 `renderDetailContent()` 中：当 `detailIsHtml` 为 true 时，仍为详情容器输出 `data-material="${detailPath}"` 与 `data-ref-content="true"`，使编辑器能绑定 contenteditable，且保存时使用 innerHTML（保留 HTML 与引用）。
- 在 `docs/js/material.js` 的 `setText()` 中：若元素带 `data-ref-content="true"`，则用 `innerHTML` 写入内容而非 `textContent`，避免 docs 加载 material 时把格式化 HTML 覆盖成纯文本。
...
## 功能增强 (2026-02-14) - Card Grid 封面/文字上下占比统一调节

### 需求
- 为 `card-grid` 增加一个统一控制项，可一次性调整所有卡片「封面图片区域」与「文字内容区域」的上下占比。
- 调整后需即时预览，并持久化到 `material`，刷新后保持一致。

### 实现方案
1. **模板层改造（按比例分区）**
   - `card-grid` 正面卡片结构由固定图片高度改为两行 `grid-template-rows`。
   - 新增节级字段 `coverMediaRatio`（图片占比，默认 57）。
   - 文本占比自动计算为 `100 - coverMediaRatio`，实现图片/文字联动。
2. **编辑模式统一控制条**
   - 在 `card-grid` 容器顶部新增控制条：`Cover` 滑杆 + 百分比显示 + `Reset`。
   - 拖动时实时更新当前区块全部卡片布局（即时预览）。
   - 松手（change）后写回 `index.<sectionId>.coverMediaRatio`。
3. **数据安全与范围限制**
   - 占比值统一做边界限制（25%~75%），避免图片/文字区过小导致排版不可用。
   - `Reset` 恢复默认 `57%`。

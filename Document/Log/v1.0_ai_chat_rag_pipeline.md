...
- 新增可切换放大/还原逻辑（toggle）：
  - 放大高度目标升级为“基线高度 + 800px”（仍受 `92vh` 上限保护）；
  - 为避免超出视口，增加上限保护（`92vh`）。
- 放大后按钮图标切换为 `minimize-2`，再次点击还原到原始高度。
- 展开/还原加入弹性动画与状态辉光：
  - 动画曲线 `cubic-bezier(0.34,1.56,0.64,1)`，展开与收回更灵动；
  - 展开时增加青色外发光，收回时还原；
  - 动画结束后自动滚动到最新消息位置。

## 增量修正：docs 与编辑页功能一致性（2026-02-22）

- 核对结果：
  - `docs` 展示页与前端编辑页均使用同一套 AI Chat 管线脚本（`docs/js/ai-chat.js` + 同源 RAG 模块）。
  - 两端模板（`docs/js/templates.js`、`js/templates.js`）已同步包含：
    - 放大按钮；
    - 发送按钮 `aria-label`；
    - 高度/阴影过渡动画。

- 修复一处回退配置不一致：
  - 编辑页 `index.html` 的内嵌回退 `index.aiChat` 原先缺少 `baseURL/endpointPath/stream/showReasoningFold` 等字段；
  - 已补齐为与 `docs` 侧一致的 Worker + Streaming + Reasoning 配置；
  - 这样在外部 `material.json` 读取失败时，编辑页行为也不会退回旧配置。

## 增量优化：Interactive Details 支持 section 级默认面（2026-02-22）

- 需求：`interactive-details` 不再固定“默认先显示 cover”，需要每个 section 可独立配置。
- 实现：
  - `docs/js/section-renderer.js`：渲染模板时把当前 section 配置对象一并传入。
  - `docs/js/templates.js`：
    - `interactiveDetailsTemplate` 新增读取 `interactiveDetailsDefaultFace`；
    - 支持 `cover` / `detail` 两种默认面；
    - 读取优先级：`config.sections[].interactiveDetailsDefaultFace` > `section data.interactiveDetailsDefaultFace` > 默认 `cover`。
  - `docs/material.json`：在 `Fusion2` section 配置中新增示例字段 `"interactiveDetailsDefaultFace": "cover"`。
- 结果：后续若有多个 `interactive-details` section，可分别设置默认进入封面或详情面，不互相影响。

## 增量同步：可视化编辑器与 docs 展示侧一致（2026-02-22）

- 背景：前一步改动已在 `docs/js/*` 生效，但编辑器侧 `js/*` 仍是旧实现，可能导致“预览与导出行为不一致”。
- 同步项：
  - `js/section-renderer.js`：`TemplateRegistry.render(...)` 同步传入 section 配置对象。
  - `js/templates.js`：
    - `interactiveDetailsTemplate` 同步支持 `interactiveDetailsDefaultFace`；
    - `TemplateRegistry.render` 同步新签名（第 5 参数 `sectionConfig`）。
- 结果：前台展示与可视化编辑界面对 `interactive-details` 默认面逻辑完全一致，避免配置冲突。

## 增量功能：Fusion 增加 Visualization Item + Learn More 新窗口跳转（2026-02-22）

- 在 `docs/material.json` 的 `index.Fusion2.items` 新增一项：
  - `title`: `Fusion Visualization`
  - `learnMoreUrl`: `../dylan.html`
- 行为改造（item 级）：
  - `docs/js/templates.js` 与 `js/templates.js`：`Learn More` 按钮支持读取 `item.learnMoreUrl`，写入 `data-learn-more-url`。
  - `docs/js/section-renderer.js` 与 `js/section-renderer.js`：
    - 若按钮存在 `data-learn-more-url`，点击时 `window.open(..., '_blank')` 新开窗口；
    - 否则保持原有翻牌到 detail 面的行为。
- 结果：只有配置了 `learnMoreUrl` 的 item 才会跳转，不影响其他 interactive-details 条目交互。

## 增量修复：GitHub Pages 跳转 dylan.html 404（2026-02-22）

- 现象：`docs` 站点点击 `Fusion Visualization` 的 `Learn More` 后，线上出现 `404 Not Found`。
- 根因：GitHub Pages 当前发布源为 `docs/`，仓库根目录的 `dylan.html` 不在发布产物内。
- 修复：
  - 新增 `docs/dylan.html`（与可视化页面内容一致，纳入 Pages 发布目录）。
  - `docs/material.json` 中该 item 的 `learnMoreUrl` 改为 `dylan.html`。
- 结果：线上从 `docs` 页面新开窗口可直接访问 `/dylan.html`，不再 404。

## 增量优化：Footer 占位文案移除 + Reference 三列排布（2026-02-22）

- 需求：
  - 去除页面结尾 footer 中占位文案：
    - `Digital Project of Physics in Society`
    - `I don't know what I can fill here...`
  - 将 `Reference` 区块改为三列排布，并保持移动端可读性。
...
- 实现方式：
  - 数据层同步清空 `footer.brandName` 与 `footer.brandTagline`（`docs/material.json` + `docs/index.html` 内嵌回退配置），避免占位文案继续渲染。
  - 模板层将 `#reference-list` 从纵向 `flex` 改为响应式网格：
    - 手机：1 列
    - 平板：2 列
    - 桌面：3 列
  - `Add Reference` 按钮设置为跨列（`col-span-full`），确保编辑模式下按钮位置稳定。
  - 适当放宽 Reference 容器最大宽度，避免桌面三列过度拥挤。

- 结果：
  - Footer 不再显示占位文本；
  - Reference 在桌面端实现 3 列展示，且在小屏自动降列，排版更紧凑清晰。

## 增量调整：Reference 强制三列 + 间距压缩（2026-02-22）

- 根据反馈将 `Reference` 区块由响应式列数改为固定 3 列（所有屏幕统一）。
- 为了让两个 reference 条目更紧凑，收缩了排版间距：
  - 列表网格间距由统一 `gap-3` 改为 `gap-x-4 gap-y-2`（行间更紧凑）；
  - 单条引用内部编号与文本间距由 `gap-3` 改为 `gap-2`。
- 改动文件：`docs/js/templates.js`。

## 增量修复：Reference 仍显示单列（2026-02-22）

- 现象：虽然模板改成了 `grid-cols-3`，页面仍表现为单列。
- 根因：当前 `docs/css/styles.css` 中不存在 `grid-cols-3` 与 `gap-x-4/gap-y-2` 这组工具类，导致类名未生效。
- 修复：
  - `docs/js/templates.js` 中将 Reference 列表改为：
    - `class="grid gap-2"`（使用已存在工具类）
    - `style="grid-template-columns: repeat(3, minmax(0, 1fr));"`（强制固定 3 列）
- 结果：Reference 在当前样式体系下可稳定显示为 3 列，同时保持更紧凑间距。

## 增量更新：MSR Quiz 从 DOCX 导入（2026-02-24）

- 需求：在 `docs` 的 MSR 部分后新增题库，题目内容尽量与 `MSR Quiz Questions.docx` 保持一致。
- 实现：
  - `docs/material.json` 的 `config.sections` 中新增 `MSRQuiz`（`template: "quiz"`），并将其顺序放在 `safety` 之后。
  - 为保持页面顺序，后续 section 的 `order` 统一顺延（`features`/`Renewable`/`sustainability`/`Fuel`/`aiChat`）。
  - 在 `index` 下新增 `MSRQuiz` 数据块，导入 4 道题。
  - 按规则处理：原文没有 `explanation` 的题目不添加 explanation 字段；仅有 3 个选项的题补充了 1 个干扰项以满足 4 选项格式。
- 结果：MSR 主内容后新增独立 quiz 区块，题干与选项主体保持 DOCX 原句，前端可正常按 quiz 模板渲染。

## 增量修复：可视化编辑器 Save All 导致功能回退（2026-02-24）

- 现象：
  - 导入可视化编辑界面后执行 Save All，出现两类回退：
    - Footer Reference 从三列恢复为单列；
    - AI Chat 从完整 RAG 管线退化为“固定模拟回复”。

- 根因定位：
  - `docs/js/templates.js` 的 Reference 布局被覆盖为纵向 `flex`，丢失 `grid-template-columns: repeat(3, minmax(0, 1fr))`。
  - `docs/js/ai-chat.js` 被覆盖为旧版简化实现（无 ScopeGuard/Retriever/LLMClient/ResponseGuard 管线）。
  - `docs/index.html` 的 AI pipeline 依赖脚本（`knowledge-index/scope-guard/embeddings/retriever/llm-client/prompt-policy/response-guard`）被移除。

- 修复方式（保留最新内容数据）：
  - 仅恢复“实现层代码”，不回退你最新的文案、题库与图片字段。
  - `docs/js/templates.js`：恢复 Reference 三列实现（固定 3 列 + Add 按钮跨列）。
  - `docs/js/ai-chat.js`：恢复完整 RAG + streaming + reasoning fold 版本。
  - `docs/index.html`：恢复 AI pipeline 脚本加载顺序，并保留当前 material 内容改动。

- 结果：
  - 最新 `docs` 材料内容（包括新增图片与 quiz 数据）继续保留；
  - Footer Reference 三列恢复；
  - AI Chat 完整链路恢复可用（ScopeGuard → Retriever → LLMClient → ResponseGuard）。
